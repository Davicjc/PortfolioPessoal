...
.project-card {{ flex: 0 0 70vw; height: 100%; position: relative; border-radius: 10px; overflow: hidden; background: #111; display: flex; flex-direction: column; justify-content: flex-end; border: 1px solid var(--muted); }}
.project-img {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; opacity: 0.5; transition: opacity 0.5s, transform 0.5s; }}
.project-card:hover .project-img {{ opacity: 0.9; transform: scale(1.05); }}
.project-info {{ position: relative; padding: 3rem; width: 100%; background: linear-gradient(to top, rgba(0,0,0,0.95), transparent); pointer-events: none; }}
...
.grid-card {{ position: relative; border-radius: 8px; overflow: hidden; border: 1px solid var(--muted); aspect-ratio: 16/10; display: flex; flex-direction: column; justify-content: flex-end; }}
.grid-img {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; opacity: 0.4; transition: all 0.4s; }}
.grid-card:hover .grid-img {{ opacity: 0.8; transform: scale(1.05); }}
.grid-info {{ position: relative; padding: 1.5rem; background: linear-gradient(to top, rgba(0,0,0,1), transparent); }}
...
function renderHome() {{
  ...
  // FEATURED PROJECTS (HORIZONTAL)
  if (DADOS.projetos && DADOS.projetos.length) {{
    var projs = publicados(DADOS.projetos).filter(p => p.destaque);
    if(projs.length === 0) projs = publicados(DADOS.projetos).slice(0, 4); // fallback
    ...
    // End card linking to all projects
    html += '<div class="project-card" style="display:flex; align-items:center; justify-content:center; background:var(--accent);">';
    html += '<a href="#/projetos" class="t-disp" style="font-size:clamp(3rem, 6vw, 5rem); color:var(--bg); text-decoration:none;">VIEW ALL<br>PROJECTS &rarr;</a>';
    html += '</div>';
...
function renderProjects() {{
  ...
  html += '<h1 class="page-title t-disp GSAP-fade">ALL PROJECTS</h1>';
  ...
  if (DADOS.projetos && DADOS.projetos.length) {{
    ...
    projs.forEach(function(p){{
      ...
      if(p.imagens && p.imagens.length) {{
        html += '<img src="' + urlImagem(p.imagens[0]) + '" class="grid-img">';
      }}
...
function renderAbout() {{
  var html = '<section class="section" style="min-height: 100vh; justify-content: flex-start;">';
  html += '<h1 class="page-title t-disp GSAP-fade" style="margin-bottom:0;">CAREER &<br>ABOUT</h1>';
...
function updateNav(hash) {{
  ...
  // English translation for nav links
...
function rotear() {{
  app.classList.remove('visible'); // fade out
  ...
  }}, 50); // slight delay to allow DOM clear and CSS transition reset
}}
...
// Overwrite __hidratar to re-render dynamically
